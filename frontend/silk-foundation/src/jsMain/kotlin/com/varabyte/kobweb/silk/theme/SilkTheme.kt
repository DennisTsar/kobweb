...
class MutableSilkTheme {
    private val _styleRules = mutableMapOf<String, SimpleStyleRule>()
    internal val styleRules: Map<String, SimpleStyleRule> = _styleRules
    private val overriddenStyleRules = mutableSetOf<String>()
...
//    private val overriddenVariants = mutableSetOf<String>()
    ...
    fun registerComponentStyle(style: ComponentStyle) {
        check(styleRules[style.name].let { it == null || it === style.styleRule }) {
            """
        ...
        _styleRules[style.name] = style.styleRule
    }
    ...
    fun replaceComponentStyle(
        ...
        check(styleRules.contains(style.name)) { "Attempting to replace a style that was never registered: \"${style.name}\"" }
        check(overriddenStyleRules.add(style.name)) { "Attempting to override style \"${style.name}\" twice" }
        _styleRules[style.name] = ComponentStyle(style.nameWithoutPrefix, extraModifiers, style.prefix, init).styleRule
    }
    ...
    fun registerComponentVariants(vararg variants: ComponentVariant) {
        variants.filterIsInstance<SimpleComponentVariant>().forEach { variant ->
            check(styleRules[variant.selector].let { it == null || it === variant.styleRule }) {
                """
                Attempting to register a second variant with a name that's already used: "${variant.selector}"
            ...
            _styleRules[variant.selector] = variant.styleRule
        }
    }
    ...
    fun replaceComponentVariant(
        ...
        check(styleRules.contains(variant.selector)) { "Attempting to replace a variant that was never registered: \"${variant.selector}\"" }
        check(overriddenStyleRules.add(variant.selector)) { "Attempting to override variant \"${variant.selector}\" twice" }
        _styleRules[variant.selector] =
            (variant.baseStyle.addVariant(
                variant.name,
                extraModifiers,
                init
            ) as SimpleComponentVariant).styleRule // TODO
    }
}
...
fun MutableSilkTheme.modifyComponentStyle(
    ...
    check(styleRules.contains(style.name)) { "Attempting to modify a style that was never registered: \"${style.name}\"" }
    val existingExtraModifiers = style.styleRule.extraModifiers
...
fun MutableSilkTheme.modifyComponentVariant(
    ...
    check(styleRules.contains(variant.selector)) { "Attempting to modify a variant that was never registered: \"${variant.selector}\"" }
    val existingExtraModifiers = variant.styleRule.extraModifiers
    val existingInit = variant.init
...
class ImmutableSilkTheme(private val mutableSilkTheme: MutableSilkTheme) {
    ...
    internal fun registerStyles(componentStyleSheet: StyleSheet) {
        ...
        mutableSilkTheme.styleRules.values.forEach { style ->
            val x = style.intoImmutableStyle()
            x.addStylesInto(componentStyleSheet)
            _componentStyles[style.selector] = x
        }
