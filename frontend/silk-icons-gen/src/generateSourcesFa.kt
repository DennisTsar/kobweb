...
fun generateSourcesFa(srcFile: Path): String {
        ...
        .map { line ->
            ...
            line.split("=", limit = 2).let { parts ->
                val category = when (parts[0]) {
                    ...
                    else -> error("Unexpected category string: ${parts[0]}")
                }
                val names = parts[1]
    ...
    iconCategories
        ...
        .let { invalidGroupings ->
            if (invalidGroupings.isNotEmpty()) {
                error("Found unexpected groupings. An icon should only be in its own category OR it can have solid and regular versions: $invalidGroupings")
            }
        }
    ...
    val iconMethodEntries = iconCategories
        .map { entry ->
            ...
            when {
                ...
                else -> error("Unhandled icon entry: $entry")
            }
        }
    // language=kotlin
    return $$"""
