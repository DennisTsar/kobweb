...
private fun position(x: EdgeXOrCenter, y: EdgeYOrCenter): String = when {
...
private fun position(x: EdgeXOffset, y: EdgeYOrCenter): String =
    if (x.offset.isDefinitelyZero()) position(x.edgeX, y) else "$x ${y.toOffset()}"

private fun position(x: EdgeXOrCenter, y: EdgeYOffset): String =
    if (y.offset.isDefinitelyZero()) position(x, y.edgeY) else "${x.toOffset()} $y"

private fun position(x: EdgeXOffset, y: EdgeYOffset): String =
    if (x.offset.isDefinitelyZero() && y.offset.isDefinitelyZero()) position(x.edgeX, y.edgeY) else "$x $y"
...
class CSSPosition private constructor(private val value: String) : StylePropertyValue {
    ...
    constructor(xAnchor: EdgeXOrCenter, yAnchor: EdgeYOrCenter) : this(position(xAnchor, yAnchor))

    constructor(xOffset: EdgeXOffset) : this(position(xOffset, Edge.CenterY))

    constructor(yOffset: EdgeYOffset) : this(position(Edge.CenterX, yOffset))
    ...
    constructor(xOffset: EdgeXOffset, yAnchor: EdgeYOrCenter) : this(position(xOffset, yAnchor))

    constructor(xAnchor: EdgeXOrCenter, yOffset: EdgeYOffset) : this(position(xAnchor, yOffset))

    constructor(xAnchor: EdgeXOffset, yAnchor: EdgeYOffset) : this(position(xAnchor, yAnchor))
