...
interface StartSettings {
    ...
    val siteModule: ModuleSources // really we just want the "module" part not the "sources" part
...
fun start(
    env: String,//ServerEnvironment,
    reuseServer: Boolean,
    @Input projectRoot: ModuleSources, // TODO: or just Path?
    siteLayout: String,//SiteLayout,
    @Input serverJar: CompilationArtifact, // TODO: or just Path?
    @Input compiledJarJustInCase: CompilationArtifact,
    @Input compiledJarJustInCase2: Path?,
    @Input  somethingElse: ModuleSources?,
) {



    val serverJar2 = somethingElse!!.sourceDirectories[1].resolve("lib/server-jvm.jar")
//    somethingElse?.sourceDirectories?.forEach {
//        println(it)
//    }
//    return
    val env = ServerEnvironment.valueOf(env)
    val siteLayout = SiteLayout.valueOf(siteLayout)
    val kobwebApplication = KobwebApplication(projectRoot.from.modulePath)

    val serverRequestsFile = ServerRequestsFile(kobwebApplication.kobwebFolder)
...
//
    ...
    stateFile.content?.let { serverState ->
        if (serverState.isRunning()) {
            ...
            if (!reuseServer) {
                error("$alreadyRunningMessage and cannot be reused for this task.")
...
//        }
        add("-jar")
//        add(serverJar2.absolutePathString())
        add(serverJar.artifact.resolve("../../_server_executableJarJvm/server-jvm-executable.jar").absolutePathString())
    }.toTypedArray()
