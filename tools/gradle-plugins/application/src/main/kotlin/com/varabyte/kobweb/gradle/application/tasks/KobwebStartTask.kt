...
abstract class KobwebStartTask @Inject constructor(
    ...
    @get:Input
    abstract val remoteDebugging: Property<AppBlock.ServerBlock.RemoteDebuggingBlock>
    ...
    private val stateFile = ServerStateFile(kobwebApplication.kobwebFolder)
//
//    @get:OutputFile
//    val x = stateFile.path

    @TaskAction
    fun execute() {
        println("remoteDebugging = ${remoteDebugging.get().enabled.get()}")
        ...
        val remoteDebuggingEnabled = (env == ServerEnvironment.DEV && remoteDebugging.get().enabled.get())
        val processParams = buildList<String> {
            ...
            if (env == ServerEnvironment.DEV && remoteDebuggingEnabled) {
                add("-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:${remoteDebugging.get().port.get()}")
            }
            add("-jar")
        ...
        stateFile.content?.let { serverState ->
            ...
            if (remoteDebuggingEnabled) {
                println("Remote debugging is enabled. You may attach a debugger to port ${remoteDebugging.get().port.get()}.")
            }
            println()
